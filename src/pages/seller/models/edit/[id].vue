...
import modelSellerApi from '../../../../api/seller/modelSellerApi.js'
import brandSellerApi from '../../../../api/seller/brandSellerApi.js'
...
const safeBrandTitle = (brand) => brand?.name?.en ?? brand?.name ?? '-'

const fetchBrands = async () => {
  try {
    const res = await brandSellerApi.getAll()
...
const fetchModel = async () => {
  ...
  try {
    const res = await modelSellerApi.getById(modelId)
    ...
    console.error(err)
    snackbarMessage.value = 'Failed to load model data.'
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await modelSellerApi.update(modelId, formData)

    snackbarMessage.value = 'Model updated successfully!'
    ...
    snackbarMessage.value = 'Failed to update model. Please check your inputs.'
    snackbarColor.value = 'error'
    snackbar.value = true
  } finally {
    loading.value = false
      ...
      <VForm @submit.prevent="handleSubmit" class="space-y-7 ma-5">
        ...
        <div>
          ...
          <VTextField
            ...
            variant="outlined"
            density="comfortable"
            placeholder="Enter model name arabic"
        ...
        <div>
          ...
          <VTextField
            ...
            variant="outlined"
            density="comfortable"
            placeholder="Enter model name english"
            prepend-inner-icon="tabler-map-pin"
