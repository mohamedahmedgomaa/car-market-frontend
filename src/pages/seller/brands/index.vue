...
const handleDelete = async () => {
  ...
  deleting.value = true
  try {
    await brandSellerApi.delete(selectedSeller.value.id)
...
const handleEdit = (id) => router.push(`/seller/brands/edit/${id}`)

const toggleActive = async (brand) => {
  try {
    brand.is_active = brand.is_active ? 0 : 1
    await brandSellerApi.update(brand.id, { is_active: brand.is_active })
  } catch (err) {
  ...
  <div class="p-4">
    <div class="flex items-center justify-between mb-6 gap-4">
      ...
      <VBtn
        ...
        @click="$router.push('/seller/brands/create')"
      >
