...
const fetchLists = async (page = 1) => {
  loading.value = true
  try {
    const res = await carFeatureAdminApi.getAll({
      page,
...
const handleDelete = async () => {
  ...
  deleting.value = true
  try {
    await carFeatureAdminApi.delete(selectedSeller.value.id)
...
const handleEdit = (id) => router.push(`/admin/car_features/edit/${id}`)

const toggleActive = async (data) => {
  try {
    data.is_active = data.is_active ? 0 : 1
    await carFeatureAdminApi.update(data.id, { is_active: data.is_active })
  } catch (err) {
  ...
  <div class="p-4">
    <div class="flex items-center justify-between mb-6 gap-4">
      ...
      <VBtn
        ...
        @click="$router.push('/admin/car_features/create')"
      >
