...
const fetchSeller = async () => {
  loading.value = true
  try {
    const res = await countryAdminApi.getById(route.params.id)
    const data = res.data.data

    // Map data to form fields
    data.value.name_ar = data.name?.ar || ''
    data.value.name_en = data.name?.en || ''
  } catch (err) {
    console.error(err)
    snackbarMessage.value = 'Failed to load data data.'
...
const handleLogoChange = (event) => {
  data.value.logo = event.target.files[0] || null
}
  ...
  try {
    ...
    for (const key in data.value) {
      if (data.value[key] !== null) {
        ...
        if (key === 'is_verified') {
          formData.append(key, data.value[key] ? '1' : '0')
        } else {
    ...
    await countryAdminApi.update(route.params.id, formData)
