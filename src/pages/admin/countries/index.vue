...
const handleDelete = async () => {
  ...
  deleting.value = true
  try {
    await countryAdminApi.delete(selectedSeller.value.id)
...
const handleEdit = (id) => router.push(`/admin/countries/edit/${id}`)

const toggleActive = async (data) => {
  try {
    data.is_active = data.is_active ? 0 : 1
    await countryAdminApi.update(data.id, { is_active: data.is_active })
  } catch (err) {
...
<template>
  <div class="p-4">
    <div class="flex items-center justify-between mb-6 gap-4">
      <h2 class="text-2xl font-semibold text-gray-100">Countries List</h2>

      <VBtn
        color="primary"
        class="rounded-lg font-medium text-white px-5 py-2 flex items-center gap-2"
        @click="$router.push('/admin/countries/create')"
      >
